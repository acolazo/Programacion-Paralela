...
int reduceMax(int size, DATATYPE *g_list, DATATYPE *g_temp, DATATYPE *g_temp_results, int * count){
    ...
    if(iterations > 1){
        reduceMax(iterations, g_temp_results, g_temp, g_temp_results, count);
    }
...
void sortBySelection(int size, DATATYPE *g_list, DATATYPE *g_temp, DATATYPE * g_temp_results, VALUETYPE * g_results){
    int count = 0;
    reduceMax(size, g_list, g_temp, g_temp_results, &count);
...
void sortBySelectionIterative(int size, DATATYPE *g_list, DATATYPE *g_temp, DATATYPE * g_temp_results, VALUETYPE * g_results){
    ...
    for(int i = size; i > 0; i--){
        reduceMax(i, g_list, g_temp, g_temp_results, &count);
...
int main(void)
{
   ...
   {   
    ...
    if (TestReduction){
        ...
        contador = reduceMax(SIZE, list_g, list_g_o, g_temp_results, &count);
        if(RECORDTIME){
            cudaEventRecord(stop);
            cudaEventSynchronize(stop);
            float milliseconds = 0;
