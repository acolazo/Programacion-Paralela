...
int receiveData(double *ptr, int source, MPI_Request *req_recv, MPI_Status *statuss, int taskid, int *pending)
{
  /*
    Receive_data
    return test_data;
...
int main(int argc, char *argv[])
{
  ...
  MPI_Request req_recv_a[PROCS];
  MPI_Request req_recv_b[PROCS];
  ...
  aw = (double *)calloc(ROWS * COLS, sizeof(double));
  if (aw == NULL)
  {
    printf("ERROR ALOCANDO MEMORIA\n");
    exit(0);
  ...
  start = MPI_Wtime();
  /* Loop */
  sendA = sendData(a, metadata.sendto[0], req_send_a, statuss, taskid, &pendingSendA);
  sendB = sendData(b, metadata.sendto[1], req_send_b, statuss, taskid, &pendingSendB);
  recvA = receiveData(aw, metadata.receivefrom[0], req_recv_a, statuss, taskid, &pendingRecvA);
  recvB = receiveData(bw, metadata.receivefrom[1], req_recv_b, statuss, taskid, &pendingRecvB);
  ...
  calculateC2 = multiplicarMatrices(aw, bw, c);
